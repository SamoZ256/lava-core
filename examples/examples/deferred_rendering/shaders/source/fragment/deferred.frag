...
void main() {
    ...
    //Check if something has been rendered at the current location. 'discard' can be pretty inefficient on some hardware, better solution would be to use the depth buffer from the gbuffer render pass to cull these pixels by using LV_COMPARE_OP_EQUAL as depthOp. Note that in order to be able to use this technique with the Vulkan backend, you need to set the layout of the depth attachment in the current subpass to LV_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL.
    if (depth == 1.0)
        discard;
