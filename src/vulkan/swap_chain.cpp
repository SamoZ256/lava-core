...
VkResult SwapChain::submitCommandBuffers(const VkCommandBuffer* buffers/*, uint32_t *imageIndex*/) {
	...
	imagesInFlight[_imageIndex] = inFlightFences[_crntFrame];

	VkSubmitInfo submitInfo = {};
	submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;

	VkSemaphore waitSemaphores[] = {imageAvailableSemaphore->semaphore(_crntFrame)};
	...
	VkSemaphore signalSemaphores[] = {renderFinishedSemaphore->semaphore(_crntFrame)};
	submitInfo.signalSemaphoreCount = 1;
	submitInfo.pSignalSemaphores = signalSemaphores;

	vkResetFences(g_vulkan_device->device(), 1, &inFlightFences[_crntFrame]);
	if (vkQueueSubmit(g_vulkan_device->graphicsQueue(), 1, &submitInfo, inFlightFences[_crntFrame]) !=
		VK_SUCCESS) {
...
void SwapChain::createSwapChain() {
	...
	uint32_t imageCount = std::max(_maxFramesInFlight, (uint8_t)swapChainSupport.capabilities.minImageCount);
	...
	image->_setBaseLayer(0);
	image->_setLayerCount(1);
	image->_setBaseMip(0);
